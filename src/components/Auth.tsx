...
import { Eye, EyeOff, Flame, X, Calendar as CalendarIcon, ChevronDown, ChevronLeft, ChevronRight, Search, Check, ArrowLeft } from 'lucide-react';
import {
  Select,
  SelectContent,
  SelectItem,
...
const Auth: React.FC<AuthProps> = ({ onAuthSuccess, setAppToast }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Object.keys(formErrors).length > 0) {
      ...
      }      if (formErrors.password && formErrors.password.includes('mínimo')) {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!resetEmail.trim()) {
      setErrors(prev => ({ ...prev, resetEmail: 'Informe seu email' }));
      return;
    }
    ...
    <div className="h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-start justify-center px-4 py-8 overflow-y-auto" style={{ WebkitOverflowScrolling: 'touch' }}>
      <div className="w-full max-w-md py-4 pt-20">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-[#403c2b] mb-4 shadow-lg border border-[#3a3628]/50">
      ...
      {/* Card de Autenticação */}
        <div className="bg-slate-800/90 backdrop-blur-sm rounded-3xl p-2 sm:p-8 shadow-2xl border border-slate-700/50 overflow-hidden relative transition-all duration-500 ease-in-out">
          
          {/* View de Recuperação de Senha */}
          {showForgotPassword ? (
            <div 
              key="forgot-password-view"
              className="animate-in fade-in slide-in-from-bottom-20 duration-500 ease-out fill-mode-forward"
            >
              <div className="bg-white rounded-[1.5rem] p-8 sm:p-10 shadow-lg">
                <div className="text-center mb-8">
                  <h2 className="text-2xl font-black text-slate-900 mb-3">Esqueceu sua senha?</h2>
                ...
                <form onSubmit={handleResetPassword} className="space-y-6">
                  <div>
                    ...
                    <input
                      ...
                      className={cn(
                        "w-full px-4 py-4 bg-slate-50 border-2 rounded-2xl text-base text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-4 transition-all shadow-sm",
                        errors.resetEmail 
                          ? "border-red-100 focus:ring-red-500/10 focus:border-red-500" 
                          : "border-slate-100 focus:ring-orange-500/10 focus:border-orange-500"
                      )}
                      autoFocus
                    />
                    {errors.resetEmail && (
                      <p className="text-[13px] font-bold text-red-500 mt-2 ml-1 animate-in fade-in slide-in-from-top-1 duration-200">
                        {errors.resetEmail}
                      </p>
                    )}
                  </div>
                  ...
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="w-full py-4 bg-gradient-to-r from-orange-500 to-amber-500 text-white text-base font-black rounded-2xl shadow-xl shadow-orange-500/30 hover:shadow-orange-500/50 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
                  >
                    {isLoading ? (
                      <>
                ...
                <div className="mt-8 text-center">
                  <button
                    ...
                    className="text-sm font-bold text-slate-400 hover:text-slate-600 transition-colors flex items-center justify-center gap-2 mx-auto group"
                  >
            ...
            <div 
              key="auth-view"
              className="animate-in fade-in slide-in-from-top-4 duration-500 ease-out fill-mode-forward"
            >
              {/* Toggle Login/Signup */}
